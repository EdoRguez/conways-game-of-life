...
public class GetFutureStateHandlerTests
{
    private readonly Mock<IBoardRepository> _mockRepo = new();
    private readonly Mock<IUnitOfWork> _mockUnitOfWork = new();
    private readonly GetFutureStateHandler _handler;

    public GetFutureStateHandlerTests()
    {
        _handler = new GetFutureStateHandler(_mockRepo.Object, _mockUnitOfWork.Object);
    }
    ...
    public async Task Handle_ZeroSteps_ReturnsCurrentState()
    {
        ...
        // Assert
        Assert.True(result.IsSuccess);
        Assert.Equal(initialCells, result.Value);
        _mockUnitOfWork.Verify(u => u.SaveChangesAsync(default));
    }
}